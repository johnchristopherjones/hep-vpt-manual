...
\newglossaryentry{System Controller}{
  ...
  description={Generic term for device which is housed in slot one of a National Instruments PXI chasis.  This is almost always an \gls{Embedded Computer}.}}

\newglossaryentry{Embedded Computer}{
  name={Embedded Computer},
  description={A small form-factor general-purpose computer with some specialization for its particular embedded application.  In this case, an \texttt{NI PXI-8104} which controls the components of a PXI-bus chasis, and is specialized for running and monitoring experimental instruments.}}

\newglossaryentry{FPGA}{
  name=FPGA,
  first={Field Programmable Gate Array (FPGA)},
  description={\textbf{F}ield \textbf{P}rogrammable \textbf{G}ate \textbf{A}rray: A Reconfigurable I/O (RIO) device; essencially a programmable intergrated circuit (IC).  It can be programmed through LabVIEW (from the system controller only) to provide real-time signalling, triggering, or procesing.  See \ref{sec:eq_pxi:fpga}}}
...
\newglossaryentry{VPT VI}{
  name=VPT VI,
  description={Literally Vaccum Photo-triode Virtual Instruments; Refers to the HEP software written in LabVIEW for the National Instruments hardware.  Includes software and hardware logic}}
